/**
 * FoodCard - Single dish card with 3-image slider, name, description, price, rating, Add to Cart
 * USED BY: Home (popular dishes), Menu (dish grid)
 * PROPS: item (dish object with images[3] or image), showAddToCart (default true), onCardClick (optional)
 */
import { useState, useEffect } from 'react'
import { useCart } from '../context/CartContext'

export default function FoodCard({ item, showAddToCart = true, onCardClick }) {
  const { addToCart } = useCart()
  const images = item.images && item.images.length >= 3 ? item.images : [item.image, item.image, item.image]
  const [imgIndex, setImgIndex] = useState(0)

  useEffect(() => {
    const t = setInterval(() => setImgIndex((i) => (i + 1) % 3), 4000)
  ...
  return (
    <article
      ...
      onKeyDown={onCardClick ? (e) => { if (e.key === 'Enter' || e.key === ' ') onCardClick(item) } : undefined}
      className={`group flex flex-col h-full rounded-2xl overflow-hidden bg-[#1c1c1c] border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.25)] hover:border-[#C9A227]/30 hover:shadow-[0_8px_32px_rgba(201,162,39,0.12)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 ${onCardClick ? 'cursor-pointer' : ''}`}
    >
      {/* 3-image slider */}
      <div className="relative aspect-[4/3] overflow-hidden flex-shrink-0">
        {images.map((src, i) => (
          <img
            ...
            loading="lazy"
            decoding="async"
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 group-hover:scale-110 ${i === imgIndex ? 'opacity-100 z-[1]' : 'opacity-0 z-0'}`}
